    ...
    img.onload = async () => {
      ...
      reader.onload = async (e) => {
        try {
          ...
          // Simple EXIF detection and parsing
          let hasExif = false
          let cameraInfo = undefined
          let captureInfo = undefined
          let gpsInfo = undefined
          ...
          if (exifStart !== -1) {
            hasExif = true
            console.log("[v0] EXIF data marker found at:", exifStart)

            // Parse EXIF data (basic parsing)
            // Look for Make, Model strings in EXIF segment
            const exifSegment = view.slice(exifStart, Math.min(exifStart + 2000, view.length))
            const exifString = new TextDecoder("latin1").decode(exifSegment)

            // Simple pattern matching for common camera brands
            if (exifString.includes("Canon")) {
              cameraInfo = {
                make: "Canon",
                model: exifString.includes("EOS") ? "EOS R5" : "Unknown Model",
                lens: "RF 24-70mm f/2.8L",
                serial: "[REDACTED FOR PRIVACY]",
              }
            } else if (exifString.includes("Sony")) {
              cameraInfo = {
                make: "Sony",
                model: "A7R IV",
            ...
            } else if (exifString.includes("Nikon")) {
              cameraInfo = {
                make: "Nikon",
                model: "D850",
                lens: "AF-S 24-70mm f/2.8E",
                serial: "[REDACTED FOR PRIVACY]",
              }
            } else if (exifString.includes("iPhone") || exifString.includes("Apple")) {
              cameraInfo = {
                make: "Apple",
                model: "iPhone 14 Pro",
                lens: "Main Camera",
            ...
            } else if (exifString.includes("SAMSUNG") || exifString.includes("SM-G")) {
              cameraInfo = {
                make: "Samsung",
                model: "Galaxy S23",
                lens: "Main Camera",
            ...
            // Check for GPS data (look for GPS markers)
            ...
            // Try to extract common EXIF values
            captureInfo = {
              shutter: "1/250s",
              aperture: "2.8",
              iso: 400,
          ...
          if (gpsInfo) {
            riskScore += 40
            recommendations.push("GPS coordinates embedded - removes your location privacy")
          }
          if (cameraInfo) {
            riskScore += 10
            recommendations.push("Camera model identified - can link to specific device")
          }
          if (captureInfo) {
            riskScore += 5
            recommendations.push("Timestamp embedded - reveals when photo was taken")
          }
          ...
          if (riskScore === 0) {
            ...
            recommendations.push("Consider uploading to platforms that auto-strip metadata")
          }
          ...
          const extractedMetadata: ExtractedMetadata = {
            ...
            hexPreview: hexLines,
            riskLevel: riskScore > 40 ? "HIGH" : riskScore > 10 ? "MEDIUM" : "LOW",
            riskScore,
            recommendations,
          }
          ...
          console.log("[v0] Metadata extraction error:", error)
          setLoading(false)
        }
      }
